...
{	
	// set composite HTransform task kinematics
	/// TO DO: choose absolute or relative transformation task
	_tkin.taskKinematics().set(_robot.TARGET_BODY, _robot.Ttarget(), _robot.Tref());
	_tkinRel.taskKinematics().set(_robot.TARGET_BODY, _robot.Ttarget(), _robot.REF_BODY, _robot.Tref());
	//_tkin.taskKinematics().set(NRMK_ROBOT_NAME::TARGET_BODY, _robot.Ttarget(), NRMK_ROBOT_NAME::REF_BODY, _robot.Tref());
	...
	_waypoint.read(_robot, _tkin.taskKinematics(), WAYPOINT_FILE);
	...
	/// FIXME @20160312
	NRMKFoundation::internal::IndexSet<ExtendedKinematics::DIM> pindex; 
	pindex.addIndex(3);
	pindex.addIndex(4);
	pindex.addIndex(5);
...
void ControlNRMK_ROBOT_NAME::initController()
{
	...
	ExtendedTaskVecRel kpRel;
	ExtendedTaskVecRel kvRel;

#if !defined(_IMPEDANCE_CONTROL)
	/// TO DO: Set gains for extended task controller
	kp.head<6>() << 100, 100, 100, 100, 100, 100;
	kv.head<6>() << 20, 20, 20, 20, 20, 20;

	kp.tail<JOINT_DOF - 6>().setZero();
	kv.tail<JOINT_DOF - 6>().setConstant(5);

	kpRel.head<6>() << 100, 100, 100, 100, 100, 100;
	kvRel.head<6>() << 20, 20, 20, 20, 20, 20;

	kpRel.tail<JOINT_DOF - 6>().setZero();
	kvRel.tail<JOINT_DOF - 6>().setConstant(5);
#else	// _IMPEDANCE_CONTROL
	NRMKFoundation::Impedance<JOINT_DOF> etask_impedance;
	JointVec mass;
	JointVec damping;
	JointVec stiffness;

	mass << 1, 1, 1, 1, 1, 1, 1;
	damping << 20, 20, 20, 20, 20, 20, 5;
	stiffness << 100, 100, 100, 0, 0, 0, 0;

	etask_impedance.setMass(mass);
	etask_impedance.setDamping(damping);
	etask_impedance.setStiffness(stiffness);

	ExtendedTaskVec kf;
	etask_impedance.generateGain(kp, kv, kf);
...
void ControlNRMK_ROBOT_NAME::_compute(double t)
{
	...
	case 2:
		{
			// extended task interpolator
			ExtendedVelocity vel_des;
			ExtendedAcceleration acc_des;

			ExtendedVelocityRel vel_des_rel;
			ExtendedAccelerationRel acc_des_rel;

			switch(_robot.REF_BODY){
			
				case -1:
						_tinterpolator.traj(t, _pos_des.task, vel_des.task, acc_des.task);
						break;	

				default:
						_tinterpolator.traj(t, _pos_des_rel.task, vel_des_rel.task, acc_des_rel.task);
						break;
			}

			// null motion suppression
// 			pos_des.x.setZero();
// 			vel_des.n.setZero();
// 			acc_des.n.setZero();
...
// 			//std::cout << num_ik_steps << std::endl;
			...
			// extended task kinematics
			ExtendedPosition pos;
			ExtendedVelocity vel;
			ExtendedJacobian J;
			ExtendedJacobian Jdot;

			ExtendedPositionRel posRel;
			ExtendedVelocityRel velRel;
			ExtendedJacobianRel JRel;
			ExtendedJacobianRel JdotRel;
			...
			// extended task controller
			ExtendedTaskVec vel_ref;
			ExtendedTaskVec acc_ref;

			ExtendedTaskVecRel vel_ref_rel;
			ExtendedTaskVecRel acc_ref_rel;
...
#else			
// 			ExtendedForce force;
// 			ExtendedForce force_des;
// 			force.task.rot = _Fext.n();
// 			force.task.disp = _robot.Tref().icascade(_robot.body(NRMK_ROBOT_NAME::TARGET_BODY).T().cascade(_robot.Ttarget())).R()*_Fext.f();
// 			force_des.task.rot = _Fdes.n();
// 			force_des.task.disp = _robot.Tref().icascade(_robot.body(NRMK_ROBOT_NAME7::TARGET_BODY).T().cascade(_robot.Ttarget())).R()*_Fdes.f();

			ExtendedForce force;
			LieGroup::Wrench Fext = _robot.Ttarget().itransform(_Fext);

			force.task.n() = Fext.n();
			force.task.f() = pos.task.R()*Fext.f();

			ExtendedForce force_des;
			force_des.task.n() = _Fdes.n();
			force_des.task.f() = _Fdes.f();
...
void ControlNRMK_ROBOT_NAME::kbCommand(unsigned char key)
{
	...
	//case VK_F: // for 15
	//case VK_G: // for 16
	// do not use more because VK_H and VK_I are preassigned.
				...
				case -1:	
							{
								...
								_tinterpolator.setTraj(_pos_target.task, vel.task);
								_pos_des.task = _pos_target.task;	
							}
							break;
				...
				default:	
							{
								...
								_tinterpolator.setTraj(_pos_target_rel.task, velRel.task);
								_pos_des_rel.task = _pos_target_rel.task;	
							}
							break;		
			}
		}
			...
			switch(_robot.REF_BODY){
				...
				case -1:
					{
						...
						_tinterpolator.setInitialTraj(_t, _pos_target.task, vel.task);
						_pos_des.task = _pos_target.task;

						if (arg == VK_T)
						{
							_pos_target.task.r() = _robot.Ttask_init().r();
							_pos_target.task.R() = _robot.Ttask_init().R(); 
														}
						else
						{
							...
							for (int i = 0; i < 3; i++)
							{
								...
								if (axes & axis)
								{
									if (arg == VK_LEFT || arg == VK_RIGHT)
										_pos_target.task.r()[j] += (arg == VK_RIGHT) ? increment_disp : -increment_disp;

									if (arg == VK_UP || arg == VK_DOWN)
										_pos_target.task.r()[k] += (arg == VK_UP) ? increment_disp : -increment_disp;
								...
								if (axes & axis)
								{
									...
									if (arg == VK_LEFT || arg == VK_RIGHT)
										(_pos_target.task.R().*rot[j])((arg == VK_RIGHT) ? increment_rot : -increment_rot);

									if (arg == VK_UP || arg == VK_DOWN)
										(_pos_target.task.R().*rot[k])((arg == VK_UP) ? increment_rot : -increment_rot);
						...
						/// TO DO: modify the trajectory completion time (which is one by default)
						_tinterpolator.setTargetTraj(_t + 1, _pos_target.task);
					}
					break;
				...
				default:

					{
						...
						_tinterpolator.setInitialTraj(_t, _pos_target_rel.task, velRel.task);
						_pos_des_rel.task = _pos_target_rel.task;

						if (arg == VK_T)
						{
							_pos_target_rel.task.r() = _robot.Ttask_init().r();
							_pos_target_rel.task.R() = _robot.Ttask_init().R(); 
						}
						else
						{
							...
							for (int i = 0; i < 3; i++)
							{
								...
								{
									if (arg == VK_LEFT || arg == VK_RIGHT)
										_pos_target_rel.task.r()[j] += (arg == VK_RIGHT) ? increment_disp : -increment_disp;

									if (arg == VK_UP || arg == VK_DOWN)
										_pos_target_rel.task.r()[k] += (arg == VK_UP) ? increment_disp : -increment_disp;
									_newTargetPosAvailable = true;
								}
								...
								if (axes & axis)
								{
									...
									if (arg == VK_LEFT || arg == VK_RIGHT)
										(_pos_target_rel.task.R().*rot[j])((arg == VK_RIGHT) ? increment_rot : -increment_rot);

									if (arg == VK_UP || arg == VK_DOWN)
										(_pos_target_rel.task.R().*rot[k])((arg == VK_UP) ? increment_rot : -increment_rot);
						...
						/// TO DO: modify the trajectory completion time (which is one by default)
						_tinterpolator.setTargetTraj(_t + 1, _pos_target_rel.task);
					}
					break;
			}
		}
...
// Teaching waypoints
	...
	case USER_CMD_INSERT_WAYPOINT:
		_robot.update(true);
		_waypoint.insert(_robot, _tkin.taskKinematics()); 
	...
	case USER_CMD_EXECUTE_WAYPOINT:
		...
		else if (_mode_waypoint == 1)			
		{
			...
			_tinterpolator.setInitialTraj(_t, pos.task);
