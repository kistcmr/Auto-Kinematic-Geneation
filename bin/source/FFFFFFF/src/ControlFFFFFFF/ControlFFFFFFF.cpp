 ...
 */
#include "Controlfffffff.h"
#include "ControlfffffffCmd.h"
...
{	
	...
	//_tkin.set(fffffff::TARGET_BODY, _robot.Ttarget(), fffffff::REF_BODY, _robot.Tref());
...
void Controlfffffff::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlfffffff::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlfffffff::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlfffffff::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlfffffff::setPeriod(double dT)
{
	_delT = dT;
...
void Controlfffffff::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlfffffff::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlfffffff::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlfffffff::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlfffffff::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
