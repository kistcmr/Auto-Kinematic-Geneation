 ...
 */
#include "Controluuu.h"
#include "ControluuuCmd.h"
...
{	
	...
	//_tkin.set(uuu::TARGET_BODY, _robot.Ttarget(), uuu::REF_BODY, _robot.Tref());
...
void Controluuu::_servoOn()
{
	_servo = SERVO_ON;
}

void Controluuu::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controluuu::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controluuu::initController()
{
#if !defined(KIN_CONTROL)
...
void Controluuu::setPeriod(double dT)
{
	_delT = dT;
...
void Controluuu::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controluuu::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controluuu::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controluuu::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controluuu::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
