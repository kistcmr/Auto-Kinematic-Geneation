...
class DataSocketbb : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketbb(bb const & robot) 
...
private:
	...
	{
		for (int k = 0; k < bb::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < bb::JOINT_DOF; k++)
			data[bb::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < bb::JOINT_DOF; k++)
			data[bb::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
		...
		return bb::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return bb::JOINT_DOF*2;
#else
		return bb::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkbb.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkbb.cpp.
