...
{	
	...
	//_tkin.set(bb::TARGET_BODY, _robot.Ttarget(), bb::REF_BODY, _robot.Tref());
...
void Controlbb::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlbb::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlbb::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlbb::setPeriod(double dT)
{
	_delT = dT;
...
void Controlbb::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlbb::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlbb::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlbb::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlbb::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
