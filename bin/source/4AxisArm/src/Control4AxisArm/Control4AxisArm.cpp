 ...
 */
#include "Control4AxisArm.h"
#include "Control4AxisArmCmd.h"
...
Control4AxisArm::Control4AxisArm() 
: _robot()
, _home()
, _task()
, _tkin(_robot)
...
{	
	...
	//_tkin.set(4AxisArm::TARGET_BODY, _robot.Ttarget(), 4AxisArm::REF_BODY, _robot.Tref());
...
void Control4AxisArm::_servoOn()
{
	_servo = SERVO_ON;
}

void Control4AxisArm::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Control4AxisArm::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Control4AxisArm::initController()
{
#if !defined(KIN_CONTROL)
...
void Control4AxisArm::setPeriod(double dT)
{
	_delT = dT;
...
void Control4AxisArm::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Control4AxisArm::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Control4AxisArm::_estimate()
{
	// FIXME@20140930
...
void Control4AxisArm::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Control4AxisArm::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Control4AxisArm::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
