...
enum
{
	...
	= FourAxisArm::JOINT_DOF,
#elif !defined(_CONTROLLER)
	= FourAxisArm::JOINT_DOF*2,
#else
	= FourAxisArm::JOINT_DOF*3,
#endif
};

typedef NRMKHelper::NRMKDataSocket<NUM_DATASCOPE_SIGNALS> _DATA_SOCKET;

class DataSocketFourAxisArm : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketFourAxisArm(FourAxisArm const & robot) 
...
private:
	...
	{
		for (int k = 0; k < FourAxisArm::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < FourAxisArm::JOINT_DOF; k++)
			data[FourAxisArm::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < FourAxisArm::JOINT_DOF; k++)
			data[FourAxisArm::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return FourAxisArm::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return FourAxisArm::JOINT_DOF*2;
#else
		return FourAxisArm::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkFourAxisArm.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkFourAxisArm.cpp.
	static const unsigned int _sizeDataChannelArray;

	FourAxisArm const & _robot;
};
