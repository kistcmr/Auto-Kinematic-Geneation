 ...
 */
#include "ControlFourAxisArm.h"
#include "ControlFourAxisArmCmd.h"
...
ControlFourAxisArm::ControlFourAxisArm() 
: _robot()
, _home()
, _task()
, _tkin(_robot)
...
{	
	...
	//_tkin.set(FourAxisArm::TARGET_BODY, _robot.Ttarget(), FourAxisArm::REF_BODY, _robot.Tref());
...
void ControlFourAxisArm::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlFourAxisArm::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlFourAxisArm::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlFourAxisArm::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlFourAxisArm::setPeriod(double dT)
{
	_delT = dT;
...
void ControlFourAxisArm::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlFourAxisArm::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlFourAxisArm::_estimate()
{
	// FIXME@20140930
...
void ControlFourAxisArm::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlFourAxisArm::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlFourAxisArm::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
