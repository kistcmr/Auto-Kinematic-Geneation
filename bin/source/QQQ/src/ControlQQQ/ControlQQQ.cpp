 ...
 */
#include "ControlQQQ.h"
#include "ControlQQQCmd.h"
...
{	
	...
	//_tkin.set(QQQ::TARGET_BODY, _robot.Ttarget(), QQQ::REF_BODY, _robot.Tref());
...
void ControlQQQ::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlQQQ::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlQQQ::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlQQQ::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlQQQ::setPeriod(double dT)
{
	_delT = dT;
...
void ControlQQQ::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlQQQ::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlQQQ::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlQQQ::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlQQQ::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
