...
class DataSocketooooooo : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketooooooo(ooooooo const & robot) 
...
private:
	...
	{
		for (int k = 0; k < ooooooo::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < ooooooo::JOINT_DOF; k++)
			data[ooooooo::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < ooooooo::JOINT_DOF; k++)
			data[ooooooo::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return ooooooo::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return ooooooo::JOINT_DOF*2;
#else
		return ooooooo::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkooooooo.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkooooooo.cpp.
	static const unsigned int _sizeDataChannelArray;

	ooooooo const & _robot;
};
