 ...
 */
#include "Controlooooooo.h"
#include "ControloooooooCmd.h"
...
{	
	...
	//_tkin.set(ooooooo::TARGET_BODY, _robot.Ttarget(), ooooooo::REF_BODY, _robot.Tref());
...
void Controlooooooo::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlooooooo::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlooooooo::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlooooooo::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlooooooo::setPeriod(double dT)
{
	_delT = dT;
...
void Controlooooooo::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlooooooo::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlooooooo::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlooooooo::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlooooooo::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
