 ...
 */
#include "ControlEEE.h"
#include "ControlEEECmd.h"
...
{	
	...
	//_tkin.set(EEE::TARGET_BODY, _robot.Ttarget(), EEE::REF_BODY, _robot.Tref());
...
void ControlEEE::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlEEE::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlEEE::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlEEE::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlEEE::setPeriod(double dT)
{
	_delT = dT;
...
void ControlEEE::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlEEE::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlEEE::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlEEE::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlEEE::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
