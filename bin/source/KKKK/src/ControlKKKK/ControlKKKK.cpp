 ...
 */
#include "ControlKKKK.h"
#include "ControlKKKKCmd.h"
...
{	
	...
	//_tkin.set(KKKK::TARGET_BODY, _robot.Ttarget(), KKKK::REF_BODY, _robot.Tref());
...
void ControlKKKK::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlKKKK::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlKKKK::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlKKKK::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlKKKK::setPeriod(double dT)
{
	_delT = dT;
...
void ControlKKKK::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlKKKK::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlKKKK::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlKKKK::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlKKKK::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
