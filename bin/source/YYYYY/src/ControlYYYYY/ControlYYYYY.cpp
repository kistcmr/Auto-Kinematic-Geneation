 ...
 */
#include "ControlYYYYY.h"
#include "ControlYYYYYCmd.h"
...
{	
	...
	//_tkin.set(YYYYY::TARGET_BODY, _robot.Ttarget(), YYYYY::REF_BODY, _robot.Tref());
...
void ControlYYYYY::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlYYYYY::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlYYYYY::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlYYYYY::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlYYYYY::setPeriod(double dT)
{
	_delT = dT;
...
void ControlYYYYY::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlYYYYY::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlYYYYY::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlYYYYY::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlYYYYY::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
