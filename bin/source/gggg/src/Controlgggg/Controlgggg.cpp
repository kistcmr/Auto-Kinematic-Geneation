 ...
 */
#include "Controlgggg.h"
#include "ControlggggCmd.h"
...
{	
	...
	//_tkin.set(gggg::TARGET_BODY, _robot.Ttarget(), gggg::REF_BODY, _robot.Tref());
...
void Controlgggg::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlgggg::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlgggg::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlgggg::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlgggg::setPeriod(double dT)
{
	_delT = dT;
...
void Controlgggg::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlgggg::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlgggg::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlgggg::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlgggg::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
