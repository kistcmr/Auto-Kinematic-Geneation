 ...
 */
#include "ControlHHH.h"
#include "ControlHHHCmd.h"
...
{	
	...
	//_tkin.set(HHH::TARGET_BODY, _robot.Ttarget(), HHH::REF_BODY, _robot.Tref());
...
void ControlHHH::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlHHH::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlHHH::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlHHH::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlHHH::setPeriod(double dT)
{
	_delT = dT;
...
void ControlHHH::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlHHH::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlHHH::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlHHH::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlHHH::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
