 ...
 */
#include "ControlBBBBBBBB.h"
#include "ControlBBBBBBBBCmd.h"
...
ControlBBBBBBBB::ControlBBBBBBBB() 
: _robot()
, _home()
, _task()
, _tkin(_robot)
...
{	
	...
	//_tkin.set(BBBBBBBB::TARGET_BODY, _robot.Ttarget(), BBBBBBBB::REF_BODY, _robot.Tref());
...
void ControlBBBBBBBB::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlBBBBBBBB::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlBBBBBBBB::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlBBBBBBBB::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlBBBBBBBB::setPeriod(double dT)
{
	_delT = dT;
...
void ControlBBBBBBBB::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlBBBBBBBB::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlBBBBBBBB::_estimate()
{
	// FIXME@20140930
...
void ControlBBBBBBBB::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlBBBBBBBB::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlBBBBBBBB::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
