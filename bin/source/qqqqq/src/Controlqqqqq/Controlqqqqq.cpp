 ...
 */
#include "Controlqqqqq.h"
#include "ControlqqqqqCmd.h"
...
{	
	...
	//_tkin.set(qqqqq::TARGET_BODY, _robot.Ttarget(), qqqqq::REF_BODY, _robot.Tref());
...
void Controlqqqqq::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlqqqqq::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlqqqqq::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlqqqqq::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlqqqqq::setPeriod(double dT)
{
	_delT = dT;
...
void Controlqqqqq::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlqqqqq::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlqqqqq::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlqqqqq::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlqqqqq::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
