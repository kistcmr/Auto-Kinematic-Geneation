 ...
 */
#include "ControlPPPPP.h"
#include "ControlPPPPPCmd.h"
...
{	
	...
	//_tkin.set(PPPPP::TARGET_BODY, _robot.Ttarget(), PPPPP::REF_BODY, _robot.Tref());
...
void ControlPPPPP::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlPPPPP::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlPPPPP::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlPPPPP::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlPPPPP::setPeriod(double dT)
{
	_delT = dT;
...
void ControlPPPPP::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlPPPPP::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlPPPPP::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlPPPPP::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlPPPPP::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
