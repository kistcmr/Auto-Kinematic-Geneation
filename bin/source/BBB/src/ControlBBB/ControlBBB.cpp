 ...
 */
#include "ControlBBB.h"
#include "ControlBBBCmd.h"
...
{	
	...
	//_tkin.set(BBB::TARGET_BODY, _robot.Ttarget(), BBB::REF_BODY, _robot.Tref());
...
void ControlBBB::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlBBB::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlBBB::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlBBB::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlBBB::setPeriod(double dT)
{
	_delT = dT;
...
void ControlBBB::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlBBB::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlBBB::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlBBB::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlBBB::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
