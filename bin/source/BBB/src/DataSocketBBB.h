...
class DataSocketBBB : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketBBB(BBB const & robot) 
...
private:
	...
	{
		for (int k = 0; k < BBB::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < BBB::JOINT_DOF; k++)
			data[BBB::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < BBB::JOINT_DOF; k++)
			data[BBB::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return BBB::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return BBB::JOINT_DOF*2;
#else
		return BBB::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkBBB.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkBBB.cpp.
