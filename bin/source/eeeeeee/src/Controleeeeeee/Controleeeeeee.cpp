 ...
 */
#include "Controleeeeeee.h"
#include "ControleeeeeeeCmd.h"
...
{	
	...
	//_tkin.set(eeeeeee::TARGET_BODY, _robot.Ttarget(), eeeeeee::REF_BODY, _robot.Tref());
...
void Controleeeeeee::_servoOn()
{
	_servo = SERVO_ON;
}

void Controleeeeeee::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controleeeeeee::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controleeeeeee::initController()
{
#if !defined(KIN_CONTROL)
...
void Controleeeeeee::setPeriod(double dT)
{
	_delT = dT;
...
void Controleeeeeee::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controleeeeeee::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controleeeeeee::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controleeeeeee::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controleeeeeee::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
