 ...
 */
#include "ControlPPP.h"
#include "ControlPPPCmd.h"
...
{	
	...
	//_tkin.set(PPP::TARGET_BODY, _robot.Ttarget(), PPP::REF_BODY, _robot.Tref());
...
void ControlPPP::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlPPP::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlPPP::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlPPP::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlPPP::setPeriod(double dT)
{
	_delT = dT;
...
void ControlPPP::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlPPP::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlPPP::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlPPP::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlPPP::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
