...
enum
{
	...
	= vvvvvssss::JOINT_DOF*2,
#else
	= vvvvvssss::JOINT_DOF*3,
#endif
};

typedef NRMKHelper::NRMKDataSocket<NUM_DATASCOPE_SIGNALS> _DATA_SOCKET;

class DataSocketvvvvvssss : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketvvvvvssss(vvvvvssss const & robot) 
...
private:
	...
	{
		for (int k = 0; k < vvvvvssss::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < vvvvvssss::JOINT_DOF; k++)
			data[vvvvvssss::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < vvvvvssss::JOINT_DOF; k++)
			data[vvvvvssss::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return vvvvvssss::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return vvvvvssss::JOINT_DOF*2;
#else
		return vvvvvssss::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkvvvvvssss.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkvvvvvssss.cpp.
	static const unsigned int _sizeDataChannelArray;

	vvvvvssss const & _robot;
};
