 ...
 */
#include "Controlvvvvvssss.h"
#include "ControlvvvvvssssCmd.h"
...
Controlvvvvvssss::Controlvvvvvssss() 
: _robot()
, _home()
, _task()
, _tkin(_robot)
...
{	
	...
	//_tkin.set(vvvvvssss::TARGET_BODY, _robot.Ttarget(), vvvvvssss::REF_BODY, _robot.Tref());
...
void Controlvvvvvssss::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlvvvvvssss::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlvvvvvssss::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlvvvvvssss::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlvvvvvssss::setPeriod(double dT)
{
	_delT = dT;
...
void Controlvvvvvssss::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlvvvvvssss::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlvvvvvssss::_estimate()
{
	// FIXME@20140930
...
void Controlvvvvvssss::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlvvvvvssss::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlvvvvvssss::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
