 ...
 */
#include "ControlTTT.h"
#include "ControlTTTCmd.h"
...
{	
	...
	//_tkin.set(TTT::TARGET_BODY, _robot.Ttarget(), TTT::REF_BODY, _robot.Tref());
...
void ControlTTT::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlTTT::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlTTT::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlTTT::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlTTT::setPeriod(double dT)
{
	_delT = dT;
...
void ControlTTT::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlTTT::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlTTT::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlTTT::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlTTT::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
