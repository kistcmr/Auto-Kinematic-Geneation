...
class DataSocketTTT : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketTTT(TTT const & robot) 
...
private:
	...
	{
		for (int k = 0; k < TTT::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < TTT::JOINT_DOF; k++)
			data[TTT::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < TTT::JOINT_DOF; k++)
			data[TTT::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return TTT::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return TTT::JOINT_DOF*2;
#else
		return TTT::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkTTT.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkTTT.cpp.
