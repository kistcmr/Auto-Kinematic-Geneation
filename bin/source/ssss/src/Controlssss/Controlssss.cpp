 ...
 */
#include "Controlssss.h"
#include "ControlssssCmd.h"
...
{	
	...
	//_tkin.set(ssss::TARGET_BODY, _robot.Ttarget(), ssss::REF_BODY, _robot.Tref());
...
void Controlssss::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlssss::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlssss::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlssss::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlssss::setPeriod(double dT)
{
	_delT = dT;
...
void Controlssss::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlssss::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlssss::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlssss::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlssss::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
