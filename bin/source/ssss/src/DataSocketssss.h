...
class DataSocketssss : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketssss(ssss const & robot) 
...
private:
	...
	{
		for (int k = 0; k < ssss::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < ssss::JOINT_DOF; k++)
			data[ssss::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < ssss::JOINT_DOF; k++)
			data[ssss::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return ssss::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return ssss::JOINT_DOF*2;
#else
		return ssss::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkssss.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkssss.cpp.
