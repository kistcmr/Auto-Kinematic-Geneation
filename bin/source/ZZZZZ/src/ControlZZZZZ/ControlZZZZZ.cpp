 ...
 */
#include "ControlZZZZZ.h"
#include "ControlZZZZZCmd.h"
...
{	
	...
	//_tkin.set(ZZZZZ::TARGET_BODY, _robot.Ttarget(), ZZZZZ::REF_BODY, _robot.Tref());
...
void ControlZZZZZ::_servoOn()
{
	_servo = SERVO_ON;
}

void ControlZZZZZ::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void ControlZZZZZ::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void ControlZZZZZ::initController()
{
#if !defined(KIN_CONTROL)
...
void ControlZZZZZ::setPeriod(double dT)
{
	_delT = dT;
...
void ControlZZZZZ::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void ControlZZZZZ::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void ControlZZZZZ::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void ControlZZZZZ::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int ControlZZZZZ::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
