 ...
 */
#include "Controlvvv.h"
#include "ControlvvvCmd.h"
...
{	
	...
	//_tkin.set(vvv::TARGET_BODY, _robot.Ttarget(), vvv::REF_BODY, _robot.Tref());
...
void Controlvvv::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlvvv::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlvvv::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlvvv::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlvvv::setPeriod(double dT)
{
	_delT = dT;
...
void Controlvvv::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlvvv::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlvvv::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlvvv::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlvvv::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
