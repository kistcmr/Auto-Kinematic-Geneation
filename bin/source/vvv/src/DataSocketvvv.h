...
class DataSocketvvv : public _DATA_SOCKET
{
public:
	enum
	{
...
public:
	inline DataSocketvvv(vvv const & robot) 
...
private:
	...
	{
		for (int k = 0; k < vvv::JOINT_DOF; k++)
			data[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < vvv::JOINT_DOF; k++)
			data[vvv::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < vvv::JOINT_DOF; k++)
			data[vvv::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER
...
#if defined(KIN_CONTROL)
		return vvv::JOINT_DOF;
#elif !defined(_CONTROLLER)
		return vvv::JOINT_DOF*2;
#else
		return vvv::JOINT_DOF*3;
#endif
	}

private:
	/// TO DO: Define the proper data channel format array in NRMKFrameworkvvv.cpp.
	static const unsigned int _dataChannelArray[];

	/// TO DO: Define the size of actual _dataChannelArray in NRMKFrameworkvvv.cpp.
