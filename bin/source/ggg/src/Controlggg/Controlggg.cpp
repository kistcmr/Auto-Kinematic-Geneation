 ...
 */
#include "Controlggg.h"
#include "ControlgggCmd.h"
...
{	
	...
	//_tkin.set(ggg::TARGET_BODY, _robot.Ttarget(), ggg::REF_BODY, _robot.Tref());
...
void Controlggg::_servoOn()
{
	_servo = SERVO_ON;
}

void Controlggg::_servoHold()
{
	// break joints and ..
	_servo = SERVO_HOLD;
}

void Controlggg::_servoOff()
{
	_robot.tau().setZero();
	_servo = SERVO_OFF;
...
void Controlggg::initController()
{
#if !defined(KIN_CONTROL)
...
void Controlggg::setPeriod(double dT)
{
	_delT = dT;
...
void Controlggg::update(double t)
{
	if (_servo == SERVO_ON)
	{
...
void Controlggg::reflectJointState()
{
#if defined(KIN_CONTROL)
...
void Controlggg::_compute(double t)
{
	JointVec  qdotref;
	JointVec  qddotref;
...
void Controlggg::kbCommand(unsigned char key)
{
	switch (key)
	{
	case VK_TAB:
...
int Controlggg::command(const short& cmd, const int& arg)
{
	if (_servo != SERVO_ON)
		return -1;
