...
private:
	...
	virtual int _updatePacketData(float * const data)
	{
		for (int k = 0; k < Arm3::JOINT_DOF; k++)
			_fdata[k] = static_cast<float>(_robot.q()[k]*RADIAN);

#if !defined(KIN_CONTROL)			
		for (int k = 0; k < Arm3::JOINT_DOF; k++)
			_fdata[Arm3::JOINT_DOF + k] = static_cast<float>(_robot.qdot()[k]);

#if defined(_CONTROLLER)
		for (int k = 0; k < Arm3::JOINT_DOF; k++)
			_fdata[Arm3::JOINT_DOF*2 + k] = static_cast<float>(_robot.tau()[k]);
#endif	// _CONTROLLER

#endif	// KIN_CONTROL

		memcpy(data, _fdata, sizeof(float)*NUM_DATASCOPE_SIGNALS);
...
private:
	float _fdata[NUM_DATASCOPE_SIGNALS];
