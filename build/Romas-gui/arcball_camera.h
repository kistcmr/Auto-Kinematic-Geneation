...
// * pan:
//     * 1 means pan, 0 means no pan
// * rotate:
//     * 1 means rotate, 0 means no rotate
// * delta_scroll_ticks:
    ...
    unsigned int flags)
{
    ...
    // if rotate isnot zero, then mouse movements implement rotation.
    if (rotate)
    {
...
end_rotate:
    ...
    // if pan isnot 0, then mouse movements implement translation
    if (pan)
    {
        int dx = px_x0 - px_x1;
        int dy = -(px_y0 - px_y1);
        ...
        float pan_delta[3] = {
			...
			pan_speed * (dx * across[0] + dy * up[0]),
			pan_speed * (dx * across[1] + dy * up[1]),
			pan_speed * (dx * across[2] + dy * up[2]),
		};
